...
def test_login_and_me_work_with_token(client):
    ...
    login = client.post(
        "/api/auth/login", json={"email": "x@y.com", "password": "abc12345"}
    )
...
def test_auth_empty_fields(client):
    register = client.post("/api/auth/register", json={"email": "", "password": ""})

    assert register.status_code == 400
    assert register.get_json() == {"error": "email and password are required"}

    login = client.post("/api/auth/login", json={"email": "", "password": ""})
